...
#elif __GNUC__
typedef char __avl_node_align[!!(__alignof__(avl_node_t) > 4) * 2 - 1];
#else
/* clang-format off */
typedef char __avl_node_align[
    !!(offsetof(struct { char c; avl_node_t t; }, t) > 4) * 2 - 1];
/* clang-format on */
#endif

static int avl_abs(int n)
{
    int mask = (unsigned) n >> ((sizeof(int) * 8) - 1);
    return (mask + n) ^ mask;
}
...
static inline avl_node_t *avl_rotate_lr(avl_node_t *parent, int pfactor)
{
    ...
    if (child->right) {
        ...
        parent->left =
            avl_rotate_rr(child, avl_get_factor(child), &cfactor, &difference);
    } else {
...
static inline avl_node_t *avl_rotate_rl(avl_node_t *parent, int pfactor)
{
    ...
    if (child->left) {
        ...
        parent->right =
            avl_rotate_ll(child, avl_get_factor(child), &cfactor, &difference);
    } else {
        cfactor = avl_get_factor(child);
    }
...
avl_node_t *avl_search(avl_tree_t *tree,
                       avl_node_t *node,
