...
#define __has_feature(x) 0
    ...
    (__has_feature(c_static_assert) && __has_feature(c_alignof))
_Static_assert(_Alignof(rb_node_t) >= 4, "Incompatible rb_node_t alignment");
#elif __GNUC__
typedef char __rb_node_align[!!(__alignof__(rb_node_t) > 4) * 2 - 1];
#else
/* clang-format off */
typedef char __rb_node_align[
    !!(offsetof(struct { char c; rb_node_t t; }, t) > 4) * 2 - 1];
/* clang-format on */
#endif
...
static inline void rb_link_node(rb_node_t *node,
                                rb_node_t *parent,
                                rb_node_t **link)
{
...
static bool rb_erase_left(rb_node_t *node, rb_node_t *parent, rb_tree_t *tree)
{
...
static bool rb_erase_right(rb_node_t *node, rb_node_t *parent, rb_tree_t *tree)
{
...
static void rb_erase_color(rb_node_t *node, rb_node_t *parent, rb_tree_t *tree)
{
    while ((!node || rb_is_black(node)) && node != tree->root) {
        if (parent->left == node && rb_erase_left(node, parent, tree))
            break;
        if (rb_erase_right(node, parent, tree))
            break;
    }
    if (node)
        rb_set_black(node);
...
static rb_node_t *rb_insert_try(rb_tree_t *tree,
                                rb_node_t *node,
